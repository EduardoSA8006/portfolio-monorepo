...
async def verify_mfa(
    ...
    try:
        ...
        await _record_event(event_type="login_mfa_failed", reason="challenge_invalid", ip=client_ip, user_agent=user_agent)
    ...
    if user is None or not user.is_active or not user.totp_enabled or not user.totp_secret_enc:
        ...
        await _record_event(event_type="login_mfa_failed", user_id=user_id, reason="state_changed", ip=client_ip, user_agent=user_agent)
    ...
    if secret is None:
        ...
        await mfa_store.revoke_challenge(redis, challenge_token)
        await _record_event(event_type="login_mfa_failed", user_id=user_id, reason="decrypt_failed", ip=client_ip, user_agent=user_agent)
        raise TOTPInvalidError()

    if not verify_totp_code(secret, code):
        logger.info("login.mfa_bad_code", extra={"user_id": user_id, "ip": client_ip})
        await _record_event(event_type="login_mfa_failed", user_id=user_id, reason="bad_code", ip=client_ip, user_agent=user_agent)
        raise TOTPInvalidError()
    ...
    if not await mfa_store.claim_code(redis, user_id, code):
        ...
        await _record_event(event_type="login_mfa_failed", user_id=user_id, reason="replay", ip=client_ip, user_agent=user_agent)
        raise TOTPInvalidError()
    ...
    await _record_event(event_type="login_success", user_id=user_id, reason="mfa", ip=client_ip, user_agent=user_agent)
...
async def logout(
    ...
    await _record_event(event_type="logout", user_id=user_id, ip=client_ip, user_agent=user_agent)
...
async def clear_all_sessions(
    ...
    await _record_event(event_type="sessions_cleared", user_id=user_id, ip=client_ip, user_agent=user_agent)
...
async def enroll_totp(
    ...
    await db.commit()
    await _record_event(event_type="totp_enroll_started", user_id=user_id, ip=client_ip, user_agent=user_agent)
    return secret, uri
...
async def confirm_totp(
    ...
    if secret is None or not verify_totp_code(secret, code):
        await _record_event(event_type="totp_confirm_failed", user_id=user_id, ip=client_ip, user_agent=user_agent)
        raise TOTPInvalidError()

    user.totp_enabled = True
    await db.commit()
    await _record_event(event_type="totp_enabled", user_id=user_id, ip=client_ip, user_agent=user_agent)
...
async def disable_totp(
    ...
    secret = decrypt_totp_secret(user.totp_secret_enc)
    if secret is None or not verify_totp_code(secret, code):
        await _record_event(event_type="totp_disable_failed", user_id=user_id, ip=client_ip, user_agent=user_agent)
        raise TOTPInvalidError()
    ...
    await db.commit()
    await _record_event(event_type="totp_disabled", user_id=user_id, ip=client_ip, user_agent=user_agent)
